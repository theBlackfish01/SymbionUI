...
const CrowdfundingProjectBox = ({ props }: { props: CrowdfundingProject }) => {
  ...
  const handleDeposit = async () => {
    const tx = await contract.depositFunds_Crowdfunding(props.projectId, {
      value: (Number(depositAmount) * 1000000000000000000).toString(),
    });
    await tx.wait();
    setdepositAmount("");
    ...
    <>
      {props.projectId != 0 ? (
        <div className="w-full bg-[#fff2] rounded-xl shadow-lg shadow-black p-5 md:p-10">
          ...
          <p>
            Merchant Depost: {props.merchantDeposit / 1000000000000000000} Eth
          </p>
          <p>Amount Raised: {props.amountRaised / 1000000000000000000} Eth</p>
          <p>Deadline: {props.deadline} days</p>
          <p>
            Profit Sharing Ratio:{" "}
            {props.profitSharingRatio
              ? props.profitSharingRatio
              : "Not yet decided"}
          </p>
          <p>
            Investment Round Active:{" "}
            {props.investmentRoundIsActive == true ? "Yes" : "No"}
          </p>
          <p>
            Profit Distribution Done:{" "}
            {props.profitDistributionIsDone == true ? "Yes" : "No"}
          </p>
          <input
